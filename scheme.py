...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 5
        y = scheme_eval(first, env)
        check_procedure(y)
        if isinstance(y, Procedure):
            x = rest.map(lambda z: scheme_eval(z, env))
            return scheme_apply(y, x, env)
        # END PROBLEM 5
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 10
        new_def = LambdaProcedure(target.second, expressions.second, env)
        env.define(target.first, new_def)
        return target.first
        # END PROBLEM 10
    else:
...
def make_let_frame(bindings, env):
    ...
    def helper_syms(bindings, env):
        if bindings is nil:
            return nil
        else:
            check_form(bindings.first, 2)
            curr_pair = bindings.first
            curr_sym = curr_pair.first
            return Pair(curr_sym, helper_syms(bindings.second, env))
    def helper_vals(bindings, env):
        if bindings is nil:
            return nil
        else:
            check_form(bindings.first, 2)
            curr_pair = bindings.first
            curr_val = scheme_eval(curr_pair.second.first, env)
            return Pair(curr_val, helper_vals(bindings.second, env))
    syms = helper_syms(bindings, env)
    vals = helper_vals(bindings, env)
    check_formals(syms)
    new_env = env.make_child_frame(syms, vals)
...
def do_mu_form(expressions, env):
    ...
    check_formals(formals)
    # BEGIN PROBLEM 16
    new_mu = MuProcedure(formals, expressions.second)
    return new_mu
    # END PROBLEM 16
